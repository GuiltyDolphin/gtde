...
(cl-defmethod gtde--build-db-from-files ((pt (eql json)) files)
  ...
  (let* ((tables (-map #'gtde-json--read-file files))
         (config-entries (--filter (eq t (gethash "GTDE_IS_CONFIG" it)) (--map (gtde-json--gethash-first it tables) (gtde-json--combined-keys tables))))
         (configs (--map (apply #'gtde-definst #'gtde--config (gtde--parse-entry-properties-no-config pt #'gtde--config nil it)) config-entries)))
...
(cl-defmethod gtde--write-item-to-file ((pt (eql json)) file item)
  ...
  (with-temp-buffer
    ...
    (let* ((table (json-parse-buffer))
      ...
      (gtde-json--write-to-object item item-obj)
